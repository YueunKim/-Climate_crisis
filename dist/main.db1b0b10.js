...
},{"buffer":"node_modules/buffer/index.js"}],"assets/main.js":[function(require,module,exports) {
"use strict";
...
(function () {
  //main2 글자 하이라이트
  var highlightElems = document.querySelectorAll('.text-highlight');
  ...
  var currentItem = graphicElems[0]; // 현재 visible인 .graphic-item
  var ioIndex;
  var io = new IntersectionObserver(function (entries, observer) {
    ioIndex = entries[0].target.dataset.index * 1;
  });
  for (var _i = 0; _i < stepElems.length; _i++) {
    io.observe(stepElems[_i]);
    stepElems[_i].dataset.index = _i;
    graphicElems[_i].dataset.index = _i;
  }
  function activate() {
    //visible 추가
  ...
  window.addEventListener('scroll', function () {
    //intro 마우스 사라지기
    var iconScrollElem = document.querySelector('.icon_scroll');
    var iconScrollOffset = iconScrollElem.getBoundingClientRect().top;
    iconScrollElem.style.cssText = window.scrollY > iconScrollOffset ? 'opacity:0; transform: translateY(100);' : 'opacity:1; transform: translateY(0px);';

    //main1 글자 올라오기
    var txtupElem = document.querySelector('#txt_up');
    var txtupOffset = txtupElem.getBoundingClientRect().top + window.pageYOffset - 800;
    txtupElem.style.cssText = window.scrollY > txtupOffset ? 'opacity:1; transform: translate3d(0px, 0px, 0px);' : 'opacity:0; transform: translate(0px, 100px);';
    ...
    //main2 타이틀 옆에서 나타나기
    var titleElem = document.querySelectorAll('.main2_tit');
    var eleOffsetArr = [];
    titleElem.forEach(function (e) {
      var eleOffset = e.getBoundingClientRect().top + window.pageYOffset - 800;
      eleOffsetArr.push(eleOffset);
    });
    eleOffsetArr.forEach(function (offset, idx) {
      titleElem[idx].style.cssText = window.scrollY > offset ? 'opacity:1; transform: translateX(0);' : 'opacity:0; transform: translateX(50px);';
    });

    //main2 글자 하이라이트
    var highlight = document.getElementsByClassName("article_txt");
    gsap.registerPlugin(ScrollTrigger);
    gsap.utils.toArray(".text-highlight").forEach(function (highlight) {
      if (highlight.dataset.index == 0 || highlight.dataset.index == 1) {
        ScrollTrigger.create({
          ...
          onEnter: function onEnter() {
            return highlight.classList.add("active1");
          }
        });
        ...
        ScrollTrigger.create({
          ...
          scrub: 1,
          onEnter: function onEnter() {
            return highlight.classList.add("active2");
          }
        });
      } else {
        ScrollTrigger.create({
          ...
          scrub: 1,
          onEnter: function onEnter() {
            return highlight.classList.add("active3");
          }
        });
      }
    });
    ...
    var step;
    var boundingRect;
    for (var _i2 = ioIndex - 1; _i2 < ioIndex + 3; _i2++) {
      step = stepElems[_i2];
      if (!step) continue;
    ...
    var elOffset = el.getBoundingClientRect().top + window.pageYOffset - 800;
    if (window.scrollY > elOffset) {
      var timer = setTimeout(type, 1500);
      clearTimeout(timer);
    }
